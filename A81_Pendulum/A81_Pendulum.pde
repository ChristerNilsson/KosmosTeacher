...
// http://openprocessing.org/sketch/205615
//
...
void draw() {
  ...
  if (line3Button.state) drawLine(3, color(230, 180, 70));
  if (line2Button.state) drawLine(2, color(230, 115, 70));
  if (line1Button.state) drawLine(1, color(230, 70, 65));
...
class Button {
  ...
  boolean over() {
    return (mouseX >= x -33 && mouseX <= x + w + 6 + labelWidth && mouseY >= y && mouseY <= y + h);
  }
}
void setBobColor() {
  // stroke(10);
...
void keyPressed() {
  if (key=='1') line1Button.state =! line1Button.state;
  if (key =='2') line2Button.state =! line2Button.state;
  if (key =='3') line3Button.state =! line3Button.state;
  if (key =='`'|| key=='0') ballButton.state =! ballButton.state;
  if (key ==' ' || key == 'p' || key == 'P') pauseButton.state =! pauseButton.state;
  if (key == 's' || key=='S') speedButton.state =! speedButton.state;
  if (key =='n' || key == 'N') numBallsButton.state =! numBallsButton.state;
  if (key == 'l' || key == 'L') labelButton.state =! labelButton.state;
  if (key == 'd' || key == 'D') debugOn =! debugOn;
  if (key =='r' || key == 'R') saveFrame();
  if (key == 'q'|| key == 'Q') alignSin =! alignSin;
}
